...
public class Solution {
    public int[] TopKFrequent(int[] nums, int k) { 
        ...
        // list of list of ints called freq
        List<int>[] freq = new List<int>[nums.Length+1];
        // instantiate the list of lists
        for(int i = 0; i < freq.Length; i++) {
            freq[i] = new List<int>();
        }
        // loop over nums and add each number to dict. if already exists, increment
        for(int i = 0; i < nums.Length; i++) {
            if(!dict.ContainsKey(nums[i])) dict.Add(nums[i], 0);
            dict[nums[i]]++;
        }
        // for each value in the dict, add it to the list of lists 
